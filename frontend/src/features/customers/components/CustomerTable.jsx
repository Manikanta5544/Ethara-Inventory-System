  ...
  return (
    <div className="card">
      <div className="flex items-center justify-between p-4 border-b border-border">
        <p className="text-sm text-ink-secondary">{data?.total ?? 0} customer{data?.total !== 1 ? "s" : ""}</p>
        ...
        <>
          <div className="overflow-x-auto max-h-[560px] overflow-y-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b border-border">{["Name","Email","Phone","Joined","Actions"].map((h)=>(<th key={h} scope="col" className="sticky top-0 z-10 bg-surface text-left text-xs font-medium text-ink-muted uppercase tracking-wider px-4 py-2.5">{h}</th>))}</tr></thead>
              <tbody className="divide-y divide-border">
                {customers.map((c) => (
                  <tr key={c.id} className="hover:bg-elevated/60 transition-colors duration-150">
                    ...
                    <td className="px-4 py-2.5 text-ink-muted text-xs">{new Date(c.created_at).toLocaleDateString()}</td>
                    <td className="px-4 py-2.5">
                      <button className="p-1.5 rounded-lg hover:bg-danger/10 text-ink-muted hover:text-danger transition-colors duration-150" onClick={() => setDeleteId(c.id)} aria-label={`Delete ${c.name}`}>
